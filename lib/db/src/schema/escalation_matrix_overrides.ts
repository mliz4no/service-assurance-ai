...
export const escalationMatrixOverridesTable = pgTable("escalation_matrix_overrides", {
  ...
  derivedSeverity: text("derived_severity", { enum: ["low", "medium", "high", "critical"] }).notNull(),
  updatedByUserId: uuid("updated_by_user_id").references(() => usersTable.id, { onDelete: "set null" }),
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow().$onUpdate(() => new Date()),
}, (t) => ({
  uniqueScopeCell: unique("uq_matrix_scope_cell").on(t.scopeType, t.scopeId, t.impactLevel, t.urgencyLevel),
}));
