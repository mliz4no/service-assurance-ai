...
export const ticketsTable = pgTable(
  'tickets',
  {
    ...
    source: text('source', { enum: ['manual', 'email', 'api', 'controller'] })
      .notNull()
      .default('manual'),
    ...
    status: text('status', {
      ...
      .notNull()
      .default('new'),
    outageType: text('outage_type', { enum: ['outage', 'impairment', 'informational', 'unknown'] })
      .notNull()
      .default('unknown'),
    ...
    incidentSource: text('incident_source', { enum: ['manual', 'email', 'controller'] }).default(
      'manual',
    ),
  ...
  (table) => [
    uniqueIndex('tickets_external_source_external_id_uidx').on(table.externalSource, table.externalId),
  ],
);
