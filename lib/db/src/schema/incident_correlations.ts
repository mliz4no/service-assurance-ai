...
export const incidentCorrelationsTable = pgTable("incident_correlations", {
  ...
  deviceEventId: uuid("device_event_id").notNull().references(() => deviceEventsTable.id, { onDelete: "cascade" }),
  correlationType: text("correlation_type", { enum: ["trigger", "related", "recovery"] }).notNull().default("trigger"),
