...
export const networkLinksTable = pgTable('network_links', {
  ...
  managedDeviceId: uuid('managed_device_id')
    .notNull()
    .references(() => managedDevicesTable.id, { onDelete: 'cascade' }),
  ...
  linkType: text('link_type', {
    enum: ['internet', 'mpls', 'lte', 'broadband', 'wan_uplink', 'vpn_tunnel', 'sdwan_transport'],
  }).notNull(),
  providerName: text('provider_name'),
  circuitId: text('circuit_id'),
  role: text('role', { enum: ['primary', 'backup', 'unknown'] })
    .notNull()
    .default('unknown'),
