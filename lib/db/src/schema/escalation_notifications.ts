...
export const escalationNotificationsTable = pgTable('escalation_notifications', {
  ...
  contactId: uuid('contact_id').references(() => customerContactsTable.id, {
    onDelete: 'set null',
  }),
  ...
  reason: text('reason', {
    enum: ['severity_threshold', 'duration_threshold', 'manual'],
  }).notNull(),
  durationMinutes: integer('duration_minutes').notNull().default(0),
  message: text('message').notNull(),
  status: text('status', { enum: ['simulated', 'sent', 'failed'] })
    .notNull()
    .default('simulated'),
