import { doublePrecision, pgTable, text, timestamp, uuid, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const sitesTable = pgTable(
  ...
  (table) => [
    uniqueIndex('sites_external_source_external_id_uidx').on(table.externalSource, table.externalId),
  ],
);
