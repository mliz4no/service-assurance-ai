...
import { z } from "zod";
import { useCreateService, getGetServicesQueryKey, useGetCustomers, useGetSites } from "@workspace/api-client-react";
...
export default function ServiceNew() {
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    createMutation.mutate({ data: {
      ...
      monthlyRecurringCharge: values.monthlyRecurringCharge ? Number(values.monthlyRecurringCharge) : null,
      ...
      onError: (err: any) => {
        toast({ title: "Error creating service", description: err.response?.data?.message || err.message, variant: "destructive" });
      }
    });
  }
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <FormField control={form.control} name="vendorName" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="e.g. AT&T, Lumen" {...field} /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                </div>
                ...
                <div className="pt-4 border-t border-border/50">
                  <h3 className="text-sm font-semibold mb-4 text-muted-foreground">Circuit Attributes</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField control={form.control} name="circuitId" render={({ field }) => (
                      <FormItem>
                    ...
                    <FormField control={form.control} name="bandwidth" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input placeholder="100Mbps" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={form.control} name="monthlyRecurringCharge" render={({ field }) => (
                      <FormItem>
                    ...
                    <FormField control={form.control} name="supportReference" render={({ field }) => (
                      <FormItem>
