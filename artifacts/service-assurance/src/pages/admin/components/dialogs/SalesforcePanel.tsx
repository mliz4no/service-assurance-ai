      ...
      <CardContent className="pt-4 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={savedConfigAny?.hasClientSecret ? 'Leave blank to keep existing' : 'Consumer Secret'}
                  rows={2}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={savedConfigAny?.hasPassword ? 'Leave blank to keep existing' : 'Salesforce password'}
                  rows={2}
                  className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm font-mono resize-none focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 placeholder:text-muted-foreground placeholder:font-sans"
                  autoComplete="off"
                  spellCheck={false}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ...
          <div className="p-3 bg-muted/30 rounded-md border border-border/50">
            <p className="text-xs text-muted-foreground mb-1">Accounts synced</p>
              <p className="text-lg font-bold">{loadingStatus ? '—' : (statusAny?.accountsSynced ?? 0)}</p>
          </div>
          <div className="p-3 bg-muted/30 rounded-md border border-border/50">
            <p className="text-xs text-muted-foreground mb-1">Contacts synced</p>
            <p className="text-lg font-bold">{loadingStatus ? '—' : (statusAny?.contactsSynced ?? 0)}</p>
          </div>
          <div className="p-3 bg-muted/30 rounded-md border border-border/50">
        ...
        {syncData && (
          <div className="p-3 bg-green-50 border border-green-200 rounded-md text-xs text-green-800">
            Sync complete — {syncData.accounts?.synced ?? 0} accounts, {syncData.contacts?.synced ?? 0} contacts processed.
            {(syncData.accounts?.errors?.length > 0 || syncData.contacts?.errors?.length > 0) && (
