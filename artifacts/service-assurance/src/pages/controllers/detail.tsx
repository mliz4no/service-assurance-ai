...
import { useGetController, useSyncController, useTestControllerConnection } from "@/lib/controller-hooks";
...
import { RefreshCw, Link2, CheckCircle2, XCircle, Clock, ArrowLeft, Server, Activity } from "lucide-react";
import { formatDistanceToNow, format } from "date-fns";

function pollStatusBadge(status: string | null) {
  if (status === "success") return <Badge className="bg-green-100 text-green-800 border-green-200">Success</Badge>;
  if (status === "failed") return <Badge variant="destructive">Failed</Badge>;
  if (status === "running") return <Badge className="bg-blue-100 text-blue-800 border-blue-200">Running</Badge>;
...
export default function ControllerDetailPage() {
  ...
  function handleSync() {
    syncController.mutate(id!, {
      onSuccess: () => toast({ title: "Sync started", description: "Check sync logs for progress." }),
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Link href="/controllers" className="hover:underline flex items-center gap-1"><ArrowLeft className="h-3 w-3" />Controllers</Link>
            </div>
            <h2 className="text-2xl font-bold tracking-tight">{controller.name}</h2>
            <div className="flex items-center gap-3">
              <Badge className={controller.vendor === "meraki" ? "bg-teal-100 text-teal-800 border-teal-200" : "bg-orange-100 text-orange-800 border-orange-200"}>
                {controller.vendor === "meraki" ? "Cisco Meraki" : "Fortinet"}
              </Badge>
              <span className="text-sm text-muted-foreground capitalize">{controller.type.replace(/_/g, " ")}</span>
              {controller.lastPollStatus === "success" && <CheckCircle2 className="h-4 w-4 text-green-500" />}
              {controller.lastPollStatus === "failed" && <XCircle className="h-4 w-4 text-red-500" />}
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <Button onClick={handleSync} disabled={syncController.isPending}>
              <RefreshCw className={`h-4 w-4 mr-2 ${syncController.isPending ? "animate-spin" : ""}`} />
              Sync Now
            </Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            { label: "Last Poll", value: controller.lastPolledAt ? formatDistanceToNow(new Date(controller.lastPolledAt), { addSuffix: true }) : "Never", icon: Clock },
          ].map((s) => (
            <Card key={s.label}>
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="mt-1">{controller.pollingEnabled ? `Enabled (every ${controller.pollingIntervalSeconds}s)` : "Disabled"}</div>
            </div>
            {controller.lastPollMessage && (
              <div className="col-span-2">
                <div className="text-muted-foreground">Last Poll Message</div>
                <div className="mt-1 text-muted-foreground text-xs">{controller.lastPollMessage}</div>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <Table>
            ...
            <TableBody>
              {(controller as any).recentSyncLogs?.length === 0 ? (
                <TableRow><TableCell colSpan={6} className="text-center text-muted-foreground py-8">No sync logs yet</TableCell></TableRow>
              ) : (controller as any).recentSyncLogs?.map((log: any) => (
                <TableRow key={log.id}>
                  <TableCell className="text-sm">{format(new Date(log.startedAt), "MMM d, HH:mm")}</TableCell>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Link href={`/events?controllerId=${controller.id}`} className="text-xs text-primary hover:underline">View all</Link>
          </CardHeader>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {(controller as any).recentEvents?.length === 0 ? (
                ...
                <TableRow key={evt.id}>
                  <TableCell className="font-medium text-sm max-w-xs truncate">{evt.title}</TableCell>
                  ...
                  <TableCell className="text-sm text-muted-foreground">{formatDistanceToNow(new Date(evt.occurredAt), { addSuffix: true })}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
