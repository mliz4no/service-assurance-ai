  ...
  function handleSave() {
    if (editingController) {
      ...
      createController.mutate(form, {
        onSuccess: () => {
          toast({ title: 'Controller created' });
          setDialogOpen(false);
        },
        onError: (err: any) =>
          toast({ title: 'Error', description: err.message, variant: 'destructive' }),
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Cisco Meraki and Fortinet controller integrations
            </p>
          </div>
          ...
          <Card className="border-border">
            ...
            <CardContent>
              <div className="text-3xl font-bold">
                {controllers?.filter((c) => c.pollingEnabled).length ?? 0}
              </div>
            </CardContent>
          </Card>
        </div>
              ...
              {isLoading ? (
                ...
                controllers?.map((c) => (
                  <TableRow
                    ...
                    onClick={() => setLocation(`/controllers/${c.id}`)}
                  >
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-xs ${c.pollingEnabled ? 'text-green-600' : 'text-muted-foreground'}`}
                        >
                    ...
                    <TableCell>
                      {c.lastPolledAt ? (
                        <span className="text-sm text-muted-foreground">
                          {formatDistanceToNow(new Date(c.lastPolledAt), { addSuffix: true })}
                        </span>
                      ) : (
                    ...
                    <TableCell>
                      <span className="text-sm">
                        {c.deviceCount ?? 0} devices &middot; {c.eventCount ?? 0} events
                      </span>
                    </TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1">
              ...
              <Input
                ...
                placeholder="Meraki Dashboard — Acme Corp"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Select
                  value={form.vendor}
                  onValueChange={(v) => setForm({ ...form, vendor: v as any })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="space-y-1">
              ...
              <Input
                type="password"
                value={form.apiKeyEncryptedOrPlaceholder}
                onChange={(e) => setForm({ ...form, apiKeyEncryptedOrPlaceholder: e.target.value })}
                placeholder="Enter API key or leave as placeholder"
              />
            </div>
            <div className="space-y-1">
              <Label>Organization / Tenant ID (optional)</Label>
              <Input
                value={form.organizationIdOrTenant}
                onChange={(e) => setForm({ ...form, organizationIdOrTenant: e.target.value })}
                placeholder="org-id or tenant"
              />
            </div>
            <div className="flex items-center justify-between">
              <div>
              ...
              <Switch
                ...
                onCheckedChange={(v) => setForm({ ...form, pollingEnabled: v })}
              />
            </div>
            {form.pollingEnabled && (
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setForm({ ...form, pollingIntervalSeconds: parseInt(e.target.value) || 300 })
                  }
                />
              </div>
            )}
          ...
          <DialogFooter>
            ...
            <Button
              onClick={handleSave}
              disabled={createController.isPending || updateController.isPending}
            >
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete "{deleteTarget?.name}" and all its devices, links, and
              events.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
