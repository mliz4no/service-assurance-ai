  ...
  if (!site)
    return (
      <AppLayout title="Site Not Found">
        <div className="text-center py-12 text-muted-foreground">Site not found.</div>
      </AppLayout>
    );
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
              {site.siteName}
              <span className="text-sm font-normal text-muted-foreground border px-2 py-0.5 rounded-md ml-2">
            ...
            <p className="text-muted-foreground flex items-center gap-1 text-sm mt-1">
              ...
              {site.customer ? (
                <Link href={`/customers/${site.customer.id}`} className="hover:underline">
                  {site.customer.name}
                </Link>
              ) : (
                'No customer linked'
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="col-span-1 space-y-4">
            ...
            <Card className="border-border/50 shadow-sm" data-testid="lcon-card">
              ...
              <CardContent>
                {!hasLcon ? (
                  <p className="text-sm text-muted-foreground italic" data-testid="lcon-empty">
                    No local contact on file.
                  </p>
                ) : (
                  <div className="space-y-3">
                    {site.lconName && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium" data-testid="lcon-name">
                          {site.lconName}
                        </span>
                      </div>
                    )}
              ...
              <CardContent className="p-0">
                {!site.services?.length ? (
                  <div className="p-6 text-center text-sm text-muted-foreground">
                    No services provisioned at this site.
                  </div>
                ) : (
                      ...
                      >
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            Vendor: {s.vendorName} · Circuit: {s.circuitId || 'N/A'}
                          </div>
                        </div>
