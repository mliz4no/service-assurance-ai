      ...
      <CardHeader className="pb-3 pt-4 px-5 border-b border-border/50">
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <div>
              <CardTitle className="text-sm font-semibold">Location Severity Classification</CardTitle>
          ...
          <div className="flex items-center gap-2">
            {currentImpact && (
              <Badge variant="outline" className={cn("text-xs capitalize border", BADGE_COLORS[currentImpact])}>
                {currentImpact}
              </Badge>
            )}
            {currentUrgency && (
              <Badge variant="outline" className={cn("text-xs capitalize border", BADGE_COLORS[currentUrgency])}>
                {currentUrgency}
              </Badge>
            )}
            {collapsed
      ...
      {!collapsed && (
        <CardContent className="px-5 pt-5 pb-5 space-y-5">
          <div className="flex items-start gap-1.5 text-xs text-muted-foreground">
            ...
            <span>
              Classify the typical Impact and Urgency profile for this location. These values inform how incidents affecting this site are prioritised during escalation.
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="space-y-2">
              ...
              <Select value={impact} onValueChange={(v) => setImpact(v as ImpactLevel)}>
                <SelectTrigger className={cn("w-full text-sm font-medium border capitalize", IMPACT_COLORS[impact])}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {IMPACT_OPTIONS.map((opt) => (
                    <SelectItem key={opt.value} value={opt.value} className="text-sm capitalize">
                      <span className={cn("inline-block px-1.5 py-0.5 rounded text-xs font-medium capitalize mr-1 border", BADGE_COLORS[opt.value])}>
                        {opt.label}
                      </span>
                    </SelectItem>
                  ))}
            ...
            <div className="space-y-2">
              ...
              <Select value={urgency} onValueChange={(v) => setUrgency(v as UrgencyLevel)}>
                <SelectTrigger className={cn("w-full text-sm font-medium border capitalize", URGENCY_COLORS[urgency])}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
